...
class HighFrequencyBacktester {
    ...
    getHourlyTrendFactor(date) {
        const hour = date.getHours();
        const month = date.getMonth();
        // Intraday patterns
        let hourlyBias = 0;
        if (hour >= 8 && hour <= 10)
            hourlyBias = 0.0005; // Morning pump
        if (hour >= 14 && hour <= 16)
            hourlyBias = 0.0003; // Afternoon activity
        if (hour >= 20 && hour <= 22)
            hourlyBias = -0.0002; // Evening dump
        // Monthly trends (2023 patterns)
        let monthlyBias = 0;
        if (month >= 0 && month <= 2)
            monthlyBias = 0.0002; // Q1 bull
        if (month >= 3 && month <= 5)
            monthlyBias = -0.0001; // Q2 correction
        if (month >= 6 && month <= 8)
            monthlyBias = 0; // Q3 consolidation
        if (month >= 9 && month <= 11)
            monthlyBias = 0.0001; // Q4 rally
        return hourlyBias + monthlyBias;
    }
    generateHourlyOpportunities(data, hourIndex) {
        const opportunities = [];
    ...
    generateTradingSignal(data, mlConfidence) {
        ...
        if (data.volatility > 0.01)
            signalScore += 10; // High volatility = more opportunities
        // Volume factor
        if (data.volume > 300000)
            signalScore += 10; // High volume = better signals
        // Time-based factors
        if (data.hour >= 8 && data.hour <= 16)
            signalScore += 5; // Active trading hours
        // Strategy-specific adjustments
        if (strategy === 'scalping')
            signalScore += 5; // Scalping works well in HFT
        if (strategy === 'momentum' && data.volatility > 0.012)
            signalScore += 10;
    ...
    executeHFTrade(data, opportunity) {
    ...
    exitHFTrade(trade, exitData, opportunity) {
...
//# sourceMappingURL=high-frequency-backtest.js.map