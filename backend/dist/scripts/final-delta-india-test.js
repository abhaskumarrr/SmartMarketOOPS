 ...
 */
async function makeRequest(method, endpoint, params = {}, data = null, authenticated = false) {
...
async function testDeltaIndiaComplete() {
    ...
    try {
        ...
        const contractTypes = [...new Set(products.map((p) => p.contract_type))];
        console.log(`✅ Contract types available: ${contractTypes.join(', ')}`);
        // Filter for futures and perpetuals
        const futures = products.filter((p) => (p.contract_type === 'futures' || p.contract_type === 'perpetual_futures') && p.is_active);
        console.log(`✅ Futures/Perpetual contracts: ${futures.length} active`);
        // Show top 5 contracts
        if (futures.length > 0) {
            console.log('\n📋 Top 5 Available Contracts:');
            futures.slice(0, 5).forEach((contract, index) => {
                console.log(`   ${index + 1}. ${contract.symbol} (ID: ${contract.id}) - ${contract.contract_type}`);
            });
        }
        // Find BTC contract
        const btcContract = futures.find((p) => p.symbol.includes('BTC') && p.is_active) || futures[0]; // Fallback to first available
        if (btcContract) {
            ...
            try {
                ...
                const ticker = tickers.find((t) => t.symbol === btcContract.symbol);
                if (ticker) {
...
//# sourceMappingURL=final-delta-india-test.js.map