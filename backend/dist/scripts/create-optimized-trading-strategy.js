...
exports.OptimizedTradingStrategy = void 0;
class OptimizedTradingStrategy {
    /**
     * Enhanced signal generation with market regime awareness
     */
    generateOptimizedSignal(candle, config, index, marketData) {
    ...
    calculateVolatility(data, index) {
        if (index < 20)
            return { normalized: 1, atr: 0, percentile: 50 };
        const lookback = 14;
     ...
     */
    analyzeVolumeProfile(data, index) {
        if (index < 20)
            return { strength: 1, trend: 'NEUTRAL' };
        const lookback = 20;
        const recent = data.slice(index - lookback, index);
     ...
     */
    getTechnicalSignals(candle, data, index) {
        if (index < 50)
     ...
     */
    findSupportResistance(data, index) {
        if (index < 50)
            return { support: data[index].low, resistance: data[index].high };
        const lookback = 50;
        const recent = data.slice(index - lookback, index);
        // Find pivot points
     ...
     */
    getHigherTimeframeSignal(data, index) {
        if (index < 100)
            return null;
    ...
    calculateRangeVolatility(prices) {
        const returns = [];
...
exports.OptimizedTradingStrategy = OptimizedTradingStrategy;
//# sourceMappingURL=create-optimized-trading-strategy.js.map