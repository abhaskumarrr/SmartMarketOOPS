...
async function checkRealBalance() {
    try {
        ...
        logger_1.logger.info(`🔧 Using ${credentials.testnet ? 'TESTNET' : 'LIVE'} Delta Exchange environment`);
        ...
        try {
            ...
            logger_1.logger.warn('⚠️ Could not fetch positions (likely no active positions)');
            positions = [];
        }
        ...
        if (error instanceof Error) {
            if (error.message.includes('credentials')) {
                logger_1.logger.error('💡 Make sure DELTA_EXCHANGE_API_KEY and DELTA_EXCHANGE_API_SECRET are set in your .env file');
            }
...
//# sourceMappingURL=check-real-balance.js.map