...
async function testDeltaIndia() {
    ...
    try {
        ...
        const perpetuals = products.filter((p) => p.contract_type === 'perpetual_futures' && p.is_active);
        console.log(`✅ Perpetual contracts: ${perpetuals.length} active`);
        // Find BTC perpetual
        const btcPerpetual = perpetuals.find((p) => p.symbol === 'BTCUSD' || p.symbol.includes('BTC'));
        if (btcPerpetual) {
...
//# sourceMappingURL=direct-delta-india-test.js.map