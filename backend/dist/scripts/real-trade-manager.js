...
class RealTradeManager {
    ...
    async getOpenPositions() {
        try {
            ...
            const openPositions = positions.filter(pos => pos.contracts > 0 || pos.size > 0);
    ...
    async placeStopLoss(symbol, side, amount, stopPrice) {
        try {
            ...
            const order = await this.exchange.createOrder(symbol, 'stop_market', // Stop market order
            side === 'BUY' ? 'sell' : 'buy', // Opposite side to close position
            amount, null, // No limit price for market order
            null, {
                stopPrice: stopPrice,
                timeInForce: 'GTC'
            });
    ...
    async placeTakeProfit(symbol, side, amount, price) {
        try {
            ...
            const order = await this.exchange.createOrder(symbol, 'limit', side === 'BUY' ? 'sell' : 'buy', // Opposite side to close position
            amount, price, null, {
                timeInForce: 'GTC'
            });
    ...
    async managePosition(position) {
        ...
        const hasStopLoss = openOrders.some(order => order.symbol === position.symbol && order.type.includes('stop'));
        if (!hasStopLoss && position.size > 0) {
            ...
            await this.placeStopLoss(position.symbol, position.side === 'long' ? 'BUY' : 'SELL', Math.abs(position.size), stopPrice);
        }
        ...
        if (position.unrealizedPnl > 0) {
            ...
            if (profitPercent > 0.02) { // 2% profit
                ...
                await this.placeTakeProfit(position.symbol, position.side === 'long' ? 'BUY' : 'SELL', partialSize, tpPrice);
            }
        }
    }
...
main();
//# sourceMappingURL=real-trade-manager.js.map