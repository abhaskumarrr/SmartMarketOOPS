...
exports.TrendingMarketTest = void 0;
...
class TrendingMarketTest {
    ...
    async runTest() {
        ...
        try {
            logger_1.logger.info('📈 Testing Enhanced Strategy in Trending Market...');
            ...
            logger_1.logger.info('🎉 Trending market test completed!');
        }
        catch (error) {
            logger_1.logger.error('❌ Trending market test failed:', error);
            throw error;
        }
    }
    /**
     ...
     */
    createTrendingMarketData() {
        logger_1.logger.info('📊 Creating synthetic trending market data...');
        ...
        logger_1.logger.info(`✅ Created ${dataPoints} trending market data points`);
        logger_1.logger.info(`📈 Price movement: $${startPrice.toLocaleString()} → $${currentPrice.toLocaleString()} (+${((currentPrice / startPrice - 1) * 100).toFixed(1)}%)`);
        return enhancedData;
    }
    /**
     * Create config optimized for trending markets
     */
    createTrendingConfig() {
        return {
            symbol: 'BTCUSD',
            timeframe: '1h',
    ...
    createOptimizedStrategy() {
        return (0, enhancedTrendStrategy_1.createEnhancedTrendStrategy)({
     ...
     */
    async testStrategy(config, strategy, marketData) {
        ...
        for (let i = 0; i < marketData.length; i++) {
            ...
            if (signal) {
                ...
                if (signal.confidence > 0) {
                    validSignals++;
                    logger_1.logger.info(`📊 Generated ${signal.type} signal at $${currentCandle.close.toFixed(0)} (Confidence: ${signal.confidence.toFixed(0)}%)`);
                    // Execute trade
    ...
    displayResults(result, startTime) {
        ...
        logger_1.logger.info(`   Data Points: ${result.dataPoints.toLocaleString()}`);
        ...
        logger_1.logger.info(`   Valid Signals: ${result.validSignals}`);
        ...
        if (performance.totalReturnPercent > 10) {
            logger_1.logger.info('   ✅ Strong positive returns in trending market');
        }
        if (performance.winRate > 50) {
            logger_1.logger.info('   ✅ Good win rate - strategy has edge in trends');
        }
        if (performance.sharpeRatio > 1) {
            logger_1.logger.info('   ✅ Good risk-adjusted returns');
        }
        if (performance.maxDrawdownPercent < 15) {
            logger_1.logger.info('   ✅ Controlled drawdown');
        }
        if (result.validSignals > 0) {
            logger_1.logger.info('   ✅ Strategy generated signals in trending conditions');
        }
        // Trade analysis
        if (result.trades.length > 0) {
            ...
            result.trades.forEach((trade, index) => {
                ...
                logger_1.logger.info(`   Trade ${index + 1}: ${trade.side} $${trade.pnl.toFixed(2)} (${trade.pnlPercent.toFixed(2)}%) - ${duration}h`);
            });
        }
        // Overall assessment
        logger_1.logger.info('\n⭐ OVERALL ASSESSMENT:');
    ...
    getPerformanceRating(performance) {
        ...
        if (performance.maxDrawdownPercent < 10)
            ...
            score -= 1;
        if (score >= 4)
            return '🌟 EXCELLENT - Strategy performs very well in trending markets';
        else if (score >= 2)
            return '✅ GOOD - Strategy shows promise in trending conditions';
        else if (score >= 0)
            return '⚠️ AVERAGE - Strategy needs optimization';
        else
            return '❌ POOR - Strategy requires significant improvements';
    }
}
exports.TrendingMarketTest = TrendingMarketTest;
/**
...
//# sourceMappingURL=test-trending-market.js.map