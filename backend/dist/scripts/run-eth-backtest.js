...
exports.runETHBacktest = runETHBacktest;
...
async function runETHBacktest() {
    try {
        ...
        logger_1.logger.info('🚀 Starting ETH Intelligent Trading Bot Backtest');
        logger_1.logger.info('⚡ EXTREME PARAMETERS: $10 capital with 200x leverage!');
        ...
        logger_1.logger.info(`💰 Initial Capital: $${config.initialCapital}`);
        logger_1.logger.info(`⚡ Dynamic Leverage: 200x → 100x → 50x → 20x → 10x`);
        logger_1.logger.info(`🎯 Dynamic Risk: 40% → 25% → 15% → 8% → 3%`);
        logger_1.logger.info(`📊 Symbol: ${config.symbol}`);
        logger_1.logger.info(`📅 Period: ${config.startDate} to ${config.endDate}`);
        logger_1.logger.info(`🔄 Max Positions: ${config.maxPositions}`);
        logger_1.logger.info('\n🔍 ADVANCED SIGNAL FILTERING (TARGET: 85%+ WIN RATE):');
        logger_1.logger.info('🤖 ML Confidence Filter: 85%+ required');
        logger_1.logger.info('📊 Ensemble Confidence: 80%+ required');
        logger_1.logger.info('⏰ Timeframe Alignment: 75%+ (4+ timeframes)');
        logger_1.logger.info('🌊 Regime Filter: Only trending/breakout markets');
        logger_1.logger.info('📈 Technical Filter: RSI 25-75, Volume 1.3x, Low volatility');
        logger_1.logger.info('🛡️ Risk Filter: Max 30% correlation, 15% drawdown limit');
        logger_1.logger.info('🎯 Signal Score: 85+/100 required to trade');
        logger_1.logger.info('\n🎯 RISK LADDER PHASES:');
        logger_1.logger.info('📈 Phase 1: SURVIVAL ($10-$50) - 40% risk, 200x leverage');
        logger_1.logger.info('🚀 Phase 2: GROWTH ($50-$200) - 25% risk, 100x leverage');
        logger_1.logger.info('💪 Phase 3: EXPANSION ($200-$1K) - 15% risk, 50x leverage');
        logger_1.logger.info('🛡️ Phase 4: CONSOLIDATION ($1K-$5K) - 8% risk, 20x leverage');
        logger_1.logger.info('💎 Phase 5: PRESERVATION ($5K+) - 3% risk, 10x leverage');
        ...
        logger_1.logger.info('✅ Backtest completed successfully!');
    }
    catch (error) {
...
function displayDetailedResults(results, executionTime) {
    logger_1.logger.info('\n🎯 DETAILED BACKTEST RESULTS:');
    logger_1.logger.info('═══════════════════════════════════════════════════════════');
    // Performance Summary
    logger_1.logger.info('\n📊 PERFORMANCE SUMMARY:');
    logger_1.logger.info(`💰 Starting Capital: $${results.config.initialCapital.toFixed(2)}`);
    logger_1.logger.info(`💰 Final Balance: $${results.summary.finalBalance.toFixed(2)}`);
    logger_1.logger.info(`📈 Total Return: $${results.summary.totalReturn.toFixed(2)}`);
    logger_1.logger.info(`📊 Return Percentage: ${results.summary.totalReturnPercent.toFixed(2)}%`);
    logger_1.logger.info(`⚡ With ${results.config.leverage}x leverage!`);
    // Risk Metrics
    logger_1.logger.info('\n⚠️ RISK METRICS:');
    logger_1.logger.info(`📉 Maximum Drawdown: ${results.summary.maxDrawdownPercent.toFixed(2)}%`);
    logger_1.logger.info(`⚡ Sharpe Ratio: ${results.summary.sharpeRatio.toFixed(3)}`);
    ...
    logger_1.logger.info(`✅ Winning Trades: ${results.summary.winningTrades}`);
    logger_1.logger.info(`❌ Losing Trades: ${results.summary.losingTrades}`);
    logger_1.logger.info(`🎯 Win Rate: ${results.summary.winRate.toFixed(1)}%`);
    logger_1.logger.info(`⏱️ Average Hold Time: ${results.summary.averageHoldTime.toFixed(1)} hours`);
    // P&L Analysis
    logger_1.logger.info('\n💰 P&L ANALYSIS:');
    logger_1.logger.info(`🏆 Average Win: $${results.summary.averageWin.toFixed(2)}`);
    logger_1.logger.info(`💥 Average Loss: $${results.summary.averageLoss.toFixed(2)}`);
    ...
    // Execution Info
    logger_1.logger.info('\n⚡ EXECUTION INFO:');
    logger_1.logger.info(`🕐 Execution Time: ${(executionTime / 1000).toFixed(2)} seconds`);
    logger_1.logger.info(`📊 Data Points Processed: 365 days`);
    // Performance Rating
    ...
    logger_1.logger.info(`💡 ${performanceRating.comment}`);
    // Risk Warning
    if (results.config.leverage >= 100) {
        logger_1.logger.info('\n⚠️ EXTREME LEVERAGE WARNING:');
        logger_1.logger.info('🚨 This backtest uses EXTREME leverage (200x)!');
        logger_1.logger.info('💀 Real trading with such leverage is EXTREMELY RISKY!');
        logger_1.logger.info('📚 This is for educational/research purposes only!');
    }
}
function getPerformanceRating(returnPercent, maxDrawdown) {
...
async function saveBacktestResults(results) {
    try {
        ...
        logger_1.logger.info(`💾 Results saved to: ${filepath}`);
        // Also save a summary CSV
        const csvData = generateCSVSummary(results);
        const csvFilename = `eth-backtest-summary-${timestamp}.csv`;
        const csvFilepath = path_1.default.join(resultsDir, csvFilename);
        fs_1.default.writeFileSync(csvFilepath, csvData);
        logger_1.logger.info(`📊 CSV summary saved to: ${csvFilepath}`);
    }
    catch (error) {
        logger_1.logger.error('❌ Error saving results:', error);
    }
}
...
function generatePerformanceAnalysis(results) {
    ...
    if (results.monthlyBreakdown && results.monthlyBreakdown.length > 0) {
        ...
        results.monthlyBreakdown.forEach((month) => {
            ...
            logger_1.logger.info(`${profitLoss} ${month.month}: ${month.trades} trades, $${month.pnl.toFixed(2)} P&L, ${month.winRate.toFixed(1)}% win rate`);
        });
    }
    // Regime Performance
    if (results.regimePerformance && results.regimePerformance.length > 0) {
        logger_1.logger.info('\n🌊 REGIME PERFORMANCE:');
        results.regimePerformance.forEach((regime) => {
            logger_1.logger.info(`📊 ${regime.regime}: ${regime.trades} trades, ${regime.winRate.toFixed(1)}% win rate, ${regime.avgReturn.toFixed(2)}% avg return`);
        });
    }
    // Risk Analysis
    logger_1.logger.info('\n⚠️ RISK ANALYSIS:');
    const riskScore = calculateRiskScore(results.summary);
    logger_1.logger.info(`🎯 Risk Score: ${riskScore.score}/100 (${riskScore.level})`);
    logger_1.logger.info(`💡 Risk Assessment: ${riskScore.assessment}`);
    // Recommendations
    logger_1.logger.info('\n💡 RECOMMENDATIONS:');
...
function calculateRiskScore(summary) {
    ...
    if (summary.maxDrawdownPercent > 80)
        score -= 40;
    else if (summary.maxDrawdownPercent > 50)
        score -= 20;
    else if (summary.maxDrawdownPercent > 30)
        score -= 10;
...
if (require.main === module) {
    runETHBacktest().catch(error => {
        logger_1.logger.error('❌ Backtest execution failed:', error);
        process.exit(1);
    });
}
//# sourceMappingURL=run-eth-backtest.js.map