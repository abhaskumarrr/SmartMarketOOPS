...
exports.Fast3MonthBacktest = void 0;
...
class Fast3MonthBacktest {
     ...
     */
    async runFast3MonthBacktest() {
        const startTime = Date.now();
        try {
            logger_1.logger.info('🚀 FAST 3-MONTH BACKTEST SYSTEM');
            // Mock Delta balance
            ...
            logger_1.logger.info(`📅 Period: 3 months backtest on BTC, ETH, SOL`);
            ...
            logger_1.logger.info(`🎉 Fast 3-month backtest completed in ${duration.toFixed(2)} seconds`);
        }
        catch (error) {
            logger_1.logger.error('❌ Fast 3-month backtest failed:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async runAssetBacktest(asset, tradingCapital) {
        try {
            // 3-month period
            ...
            logger_1.logger.info(`✅ ${asset} completed: ${finalTrades.length} trades, ${totalReturn.toFixed(2)}% return`);
            return {
                asset,
                config,
                signals,
            ...
            logger_1.logger.error(`❌ Failed ${asset}:`, error);
            return null;
        }
    }
    /**
     ...
     */
    generateFastSignal(candle, config) {
     ...
     */
    generateFastReport(results, realBalance, tradingCapital, startTime) {
        ...
        logger_1.logger.info(`   Mock Delta Balance: $${realBalance.toFixed(2)}`);
        logger_1.logger.info(`   Trading Capital (75%): $${tradingCapital.toFixed(2)}`);
        logger_1.logger.info(`   Leverage: 200x (Max: $${(tradingCapital * 200).toFixed(2)})`);
        logger_1.logger.info(`   Risk Per Trade: 5% ($${(tradingCapital * 0.05).toFixed(2)})`);
        logger_1.logger.info(`   Execution Time: ${totalDuration.toFixed(2)} seconds`);
        logger_1.logger.info('\n📊 PERFORMANCE SUMMARY:');
        logger_1.logger.info('   Asset   | Signals | Trades | Return | P&L | Final Balance | Max DD');
        ...
        results.forEach(result => {
            ...
            logger_1.logger.info(`   ${asset} | ${signals} | ${trades} | ${totalReturn}% | $${dollarPnL} | $${finalBalance} | ${maxDD}`);
        ...
        logger_1.logger.info('\n💼 OVERALL RESULTS:');
        logger_1.logger.info(`   Total Signals: ${totalSignals}`);
        logger_1.logger.info(`   Total Trades: ${totalTrades}`);
        logger_1.logger.info(`   Execution Rate: ${((totalTrades / totalSignals) * 100).toFixed(1)}%`);
        logger_1.logger.info(`   Total P&L: $${totalPnL.toFixed(2)}`);
        logger_1.logger.info(`   Final Balance: $${totalFinalBalance.toFixed(2)}`);
        ...
        logger_1.logger.info(`   Drawdown Hits: ${maxDrawdownHits}/${results.length}`);
        // Real balance impact
        ...
        logger_1.logger.info(`   Balance Change: ${((newBalance - realBalance) / realBalance * 100).toFixed(2)}%`);
        // Trade analysis
        logger_1.logger.info('\n📈 TRADE ANALYSIS:');
        results.forEach(result => {
            if (result.trades.length > 0) {
                ...
                logger_1.logger.info(`   ${result.asset}: ${result.trades.length} trades, ${winRate}% win rate, Avg Win: $${avgWin}, Avg Loss: $${avgLoss}`);
            }
        });
        if (totalPnL > 0) {
            logger_1.logger.info('\n🚀 SUCCESS: Profitable 3-month backtest!');
            logger_1.logger.info(`   Generated $${totalPnL.toFixed(2)} profit with 200x leverage`);
            logger_1.logger.info(`   ROI: ${((totalPnL / (tradingCapital * results.length)) * 100).toFixed(2)}%`);
            logger_1.logger.info(`   Annualized: ${(((totalPnL / (tradingCapital * results.length)) * 4) * 100).toFixed(2)}%`);
        }
        else {
            logger_1.logger.info('\n⚠️ LOSS: Strategy needs optimization');
            logger_1.logger.info(`   Lost $${Math.abs(totalPnL).toFixed(2)} over 3 months`);
        }
        logger_1.logger.info('\n🎯 READY FOR LIVE TRADING CONSIDERATION');
        logger_1.logger.info('='.repeat(120));
    }
}
exports.Fast3MonthBacktest = Fast3MonthBacktest;
/**
 * Main execution
 */
async function main() {
    ...
    try {
        ...
        logger_1.logger.error('💥 Fast backtest failed:', error);
        process.exit(1);
    }
}
...
//# sourceMappingURL=run-fast-3month-backtest.js.map