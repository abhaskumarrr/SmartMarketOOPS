...
class PerformanceAnalytics {
     ...
     */
    static calculateMetrics(trades, portfolioHistory, config) {
     ...
     */
    static generateReport(metrics, trades, config) {
        const report = `
...
`;
     ...
     */
    static getPerformanceRating(metrics) {
        let score = 0;
        const ratings = [];
        // Return rating
        if (metrics.totalReturnPercent > 50) {
            score += 2;
            ratings.push('🟢 Excellent Returns');
        }
        else if (metrics.totalReturnPercent > 20) {
            score += 1;
            ratings.push('🟡 Good Returns');
        }
        else if (metrics.totalReturnPercent > 0) {
            score += 0;
            ratings.push('🟡 Positive Returns');
        }
        else {
            score -= 1;
            ratings.push('🔴 Negative Returns');
        }
        // Sharpe ratio rating
        if (metrics.sharpeRatio > 2) {
            score += 2;
            ratings.push('🟢 Excellent Risk-Adjusted Returns');
        }
        else if (metrics.sharpeRatio > 1) {
            score += 1;
            ratings.push('🟡 Good Risk-Adjusted Returns');
        }
        else if (metrics.sharpeRatio > 0) {
            score += 0;
            ratings.push('🟡 Positive Risk-Adjusted Returns');
        }
        else {
            score -= 1;
            ratings.push('🔴 Poor Risk-Adjusted Returns');
        }
        // Win rate rating
        if (metrics.winRate > 60) {
            score += 1;
            ratings.push('🟢 High Win Rate');
        }
        else if (metrics.winRate > 40) {
            score += 0;
            ratings.push('🟡 Moderate Win Rate');
        }
        else {
            score -= 1;
            ratings.push('🔴 Low Win Rate');
        }
        // Drawdown rating
        if (metrics.maxDrawdownPercent < 10) {
            score += 1;
            ratings.push('🟢 Low Drawdown');
        }
        else if (metrics.maxDrawdownPercent < 20) {
            score += 0;
            ratings.push('🟡 Moderate Drawdown');
        }
        else {
            score -= 1;
            ratings.push('🔴 High Drawdown');
        }
        let overall = '';
        if (score >= 4)
            overall = '🌟 EXCELLENT STRATEGY';
        else if (score >= 2)
            overall = '✅ GOOD STRATEGY';
        else if (score >= 0)
            overall = '⚠️ AVERAGE STRATEGY';
        else
            overall = '❌ POOR STRATEGY';
        return `${overall}\n\n${ratings.join('\n')}`;
    }
    /**
     * Get top performing trades
     */
    static getTopTrades(trades, count) {
