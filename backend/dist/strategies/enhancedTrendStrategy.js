...
exports.EnhancedTrendStrategy = void 0;
exports.createEnhancedTrendStrategy = createEnhancedTrendStrategy;
const logger_1 = require("../utils/logger");
const events_1 = require("../types/events");
class EnhancedTrendStrategy {
    constructor(parameters) {
        this.name = 'Enhanced_Trend';
    ...
    generateSignal(data, currentIndex) {
        ...
        logger_1.logger.info(`📊 Generated ${enhancedSignal.type} signal`, {
    ...
    analyzeMarketRegime(data, currentIndex) {
        ...
        const volatility = Math.sqrt(returns.reduce((sum, ret) => sum + ret * ret, 0) / returns.length) * Math.sqrt(252); // Annualized
    ...
    analyzeTrend(data, currentIndex) {
    ...
    generateEntrySignal(data, currentIndex, trendAnalysis, marketRegime) {
        const currentCandle = data[currentIndex];
    ...
    enhanceSignal(signal, data, currentIndex, trendAnalysis, marketRegime) {
        const currentCandle = data[currentIndex];
        const indicators = currentCandle.indicators;
        ...
        const quantity = this.calculateDynamicPositionSize(currentCandle.close, confidence, trendAnalysis, marketRegime);
        // Calculate dynamic stop loss
        const stopLoss = this.calculateDynamicStopLoss(signal.type, currentCandle.close, data, currentIndex, trendAnalysis);
    ...
    calculateDynamicPositionSize(price, confidence, trendAnalysis, marketRegime) {
        if (!this.config)
            return 0;
        // Base position size
    ...
    calculateDynamicStopLoss(signalType, price, data, currentIndex, trendAnalysis) {
        ...
        const atrStopDistance = Math.max((atr / price) * 1.5, this.parameters.baseStopLoss / 100);
        return signalType === 'BUY'
...
exports.EnhancedTrendStrategy = EnhancedTrendStrategy;
// Export factory function
function createEnhancedTrendStrategy(params) {
    return new EnhancedTrendStrategy(params);
}
//# sourceMappingURL=enhancedTrendStrategy.js.map