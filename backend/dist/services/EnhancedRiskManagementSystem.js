...
exports.EnhancedRiskManagementSystem = void 0;
...
class EnhancedRiskManagementSystem {
    ...
    async initialize() {
        try {
            logger_1.logger.info('🛡️ Initializing Enhanced Risk Management System...');
            ...
            logger_1.logger.info(`🔒 Circuit Breakers: Max Drawdown ${(this.circuitBreakerConfig.maxDrawdownThreshold * 100).toFixed(0)}%, Max Volatility ${(this.circuitBreakerConfig.maxVolatilityThreshold * 100).toFixed(0)}%`);
            logger_1.logger.info(`⚡ Position Limits: Max Size ${(this.circuitBreakerConfig.maxPositionSize * 100).toFixed(0)}%, Max Exposure ${(this.circuitBreakerConfig.maxTotalExposure * 100).toFixed(0)}%`);
        }
        catch (error) {
            logger_1.logger.error('❌ Failed to initialize Enhanced Risk Management System:', error.message);
            throw error;
        }
    }
    /**
     ...
     */
    async assessTradingRisk(decision, currentPrice) {
        try {
            logger_1.logger.debug(`🔍 Assessing trading risk for ${decision.symbol} ${decision.action}`);
            ...
            if (!assessment.isAcceptable) {
                logger_1.logger.warn(`⚠️ High risk detected for ${decision.symbol}: ${(overallRiskScore * 100).toFixed(1)}%`);
                logger_1.logger.warn(`   Risk Factors: ${assessment.riskFactors.join(', ')}`);
            }
            // Record risk event
            this.recordRiskEvent('risk_assessment', `${decision.symbol} ${decision.action}`, overallRiskScore, assessment.isAcceptable ? 'approved' : 'rejected');
            return assessment;
        }
        catch (error) {
            logger_1.logger.error(`❌ Failed to assess trading risk for ${decision.symbol}:`, error.message);
            return {
                isAcceptable: false,
                riskScore: 1.0,
        ...
        try {
            ...
            for (const [id, mechanism] of this.failsafeMechanisms) {
                if (mechanism.isActive && this.isMechanismTriggered(mechanism)) {
                    ...
                    logger_1.logger.warn(`🚨 Circuit breaker triggered: ${mechanism.name}`);
                    logger_1.logger.warn(`   Current: ${mechanism.currentValue.toFixed(4)} | Threshold: ${mechanism.threshold.toFixed(4)}`);
                    // Record risk event
            ...
            logger_1.logger.error('❌ Failed to check circuit breakers:', error.message);
     ...
     */
    calculateDynamicPositionSize(baseSize, confidence, riskScore, marketVolatility) {
        try {
            ...
            logger_1.logger.debug(`📊 Dynamic position sizing: Base ${(baseSize * 100).toFixed(1)}% → Adjusted ${(adjustedSize * 100).toFixed(1)}%`);
            logger_1.logger.debug(`   Confidence: ${(confidence * 100).toFixed(0)}% | Risk: ${(riskScore * 100).toFixed(0)}% | Volatility: ${(marketVolatility * 100).toFixed(0)}%`);
            return Math.max(0.001, adjustedSize); // Minimum 0.1% position
        }
        catch (error) {
            logger_1.logger.error('❌ Failed to calculate dynamic position size:', error.message);
    ...
    updateCircuitBreakerConfig(newConfig) {
        ...
        logger_1.logger.info('🔧 Circuit breaker configuration updated');
    }
    /**
     * Enable/disable specific failsafe mechanism
     */
    toggleFailsafeMechanism(mechanismId, enabled) {
        const mechanism = this.failsafeMechanisms.get(mechanismId);
        if (mechanism) {
            mechanism.isActive = enabled;
            logger_1.logger.info(`${enabled ? '🟢' : '🔴'} Failsafe mechanism ${mechanism.name}: ${enabled ? 'ENABLED' : 'DISABLED'}`);
            return true;
        }
        return false;
    }
    ...
    async cleanup() {
        try {
            logger_1.logger.info('🧹 Cleaning up Enhanced Risk Management System...');
            // Save risk data
            await this.saveRiskData();
            await this.redis.quit();
            logger_1.logger.info('✅ Enhanced Risk Management System cleanup completed');
        }
        catch (error) {
            logger_1.logger.error('❌ Error during Enhanced Risk Management System cleanup:', error.message);
        }
    }
    ...
    initializeFailsafeMechanisms() {
        ...
        logger_1.logger.info(`🛡️ Initialized ${this.failsafeMechanisms.size} failsafe mechanisms`);
    }
    /**
     ...
     * Calculate position-specific risk
     */
    calculatePositionRisk(decision, currentPrice) {
        let riskScore = 0;
     ...
     */
    calculatePortfolioImpact(decision, currentPrice) {
        const activePositions = this.positionManager.getActivePositions();
     ...
     */
    async assessMarketConditions(symbol) {
        try {
            ...
            logger_1.logger.error(`❌ Failed to assess market conditions for ${symbol}:`, error.message);
     ...
     */
    calculateOverallRiskScore(positionRisk, portfolioImpact, marketRisk) {
     ...
     */
    identifyRiskFactors(positionRisk, portfolioImpact, marketRisk) {
        const factors = [];
        if (positionRisk > 0.7)
            factors.push('High position risk');
        if (portfolioImpact > 0.7)
            factors.push('High portfolio impact');
        if (marketRisk > 0.7)
            factors.push('Adverse market conditions');
        if (this.riskMetrics.volatilityIndex > 0.15)
            factors.push('High market volatility');
        if (this.riskMetrics.currentDrawdown < -0.1)
            factors.push('Significant drawdown');
        if (this.riskMetrics.totalExposure > 2.5)
            factors.push('High portfolio exposure');
        if (this.riskMetrics.concentrationRisk > 0.8)
            factors.push('Position concentration risk');
        return factors;
    }
    /**
     * Generate risk mitigation recommendations
     */
    generateRiskRecommendations(overallRisk, positionRisk, portfolioImpact) {
     ...
     */
    calculateMaxLeverage(riskScore) {
    ...
    async loadHistoricalData() {
        try {
            ...
            logger_1.logger.info(`📊 Loaded ${this.historicalReturns.length} historical returns and ${this.portfolioValues.length} portfolio values`);
        }
        catch (error) {
            logger_1.logger.error('❌ Failed to load historical data:', error.message);
        }
    }
    /**
     * Start risk monitoring
    ...
    startRiskMonitoring() {
        ...
        setInterval(async () => {
            try {
                ...
                logger_1.logger.error('❌ Error in risk monitoring:', error.message);
            }
        }, 30000);
        logger_1.logger.info('⏰ Risk monitoring started (30-second intervals)');
    }
    /**
    ...
    async calculateVolatilityIndex() {
        if (this.historicalReturns.length < 20)
    ...
    calculateSortinoRatio() {
        ...
        if (negativeReturns.length === 0)
            return 10; // Very high ratio if no negative returns
     ...
     */
    async calculateMarketRegimeRisk() {
        try {
            ...
            logger_1.logger.error('❌ Failed to calculate market regime risk:', error.message);
     ...
     * Calculate correlation risk (simplified)
     */
    calculateCorrelationRisk(symbol, positions) {
        ...
        if (hasCrypto && cryptoSymbols.includes(symbol))
     ...
     */
    isMechanismTriggered(mechanism) {
     ...
     */
    async executeEmergencyActions(triggeredMechanisms) {
        try {
            logger_1.logger.warn(`🚨 Executing emergency actions for ${triggeredMechanisms.length} triggered mechanisms`);
            ...
            logger_1.logger.error('❌ Failed to execute emergency actions:', error.message);
        }
    }
    /**
    ...
    async executeEmergencyStop() {
        logger_1.logger.error('🚨 EMERGENCY STOP TRIGGERED - CLOSING ALL POSITIONS');
        const activePositions = this.positionManager.getActivePositions();
        for (const position of activePositions) {
            try {
                await this.positionManager.closePosition(position.id, position.currentPrice, 'Emergency stop triggered');
                logger_1.logger.warn(`🔒 Emergency closed position: ${position.id}`);
            }
            catch (error) {
                logger_1.logger.error(`❌ Failed to emergency close position ${position.id}:`, error.message);
            }
        }
     ...
     */
    async executeCircuitBreakerActions(mechanism) {
        logger_1.logger.warn(`🔴 Circuit breaker action: ${mechanism.name}`);
     ...
     */
    async executeVolatilityControls() {
        logger_1.logger.warn('🌪️ High volatility detected - implementing controls');
     ...
     */
    async executePositionLimitActions() {
        logger_1.logger.warn('📊 Position limits exceeded - implementing controls');
    ...
    async reducePositionSizes(reductionFactor) {
        ...
        for (const position of activePositions) {
            try {
                ...
                logger_1.logger.info(`📉 Reducing position ${position.id} by ${(reductionFactor * 100).toFixed(0)}%`);
            }
            catch (error) {
                logger_1.logger.error(`❌ Failed to reduce position ${position.id}:`, error.message);
            }
        }
    }
    /**
     ...
     */
    async tightenStopLosses(tighteningFactor) {
        const activePositions = this.positionManager.getActivePositions();
        for (const position of activePositions) {
            try {
                ...
                logger_1.logger.info(`🔒 Tightened stop loss for position ${position.id}: ${position.stopLoss}`);
            }
            catch (error) {
                logger_1.logger.error(`❌ Failed to tighten stop loss for position ${position.id}:`, error.message);
            }
        }
    }
    /**
     ...
     */
    async suspendTrading(reason) {
        logger_1.logger.error(`⏸️ Trading suspended: ${reason}`);
     ...
     */
    recordRiskEvent(type, description, riskScore, action) {
        const event = {
            timestamp: Date.now(),
            type,
            description,
        ...
        logger_1.logger.debug(`📝 Risk event recorded: ${type} - ${description} (${action})`);
    }
    /**
    ...
    async saveRiskData() {
        try {
            ...
            logger_1.logger.debug('💾 Risk data saved to Redis');
        }
        catch (error) {
            logger_1.logger.error('❌ Failed to save risk data:', error.message);
        }
    }
}
exports.EnhancedRiskManagementSystem = EnhancedRiskManagementSystem;
//# sourceMappingURL=EnhancedRiskManagementSystem.js.map