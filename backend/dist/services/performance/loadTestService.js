...
// Create logger
const logger = (0, logger_1.createLogger)('LoadTestService');
...
class LoadTestService {
     ...
     */
    async runLoadTest(config, onResultCallback) {
        try {
...
export default function() {
  ...
  sleep(1);
     ...
     * @private
     */
    runK6(scriptPath, config, resultId, onResultCallback) {
        try {
            // Prepare k6 command
            const k6Process = (0, child_process_1.spawn)('k6', ['run', '--summary-export=stdout', scriptPath]);
            let output = '';
            let errorOutput = '';
            // Collect stdout
            ...
            k6Process.on('close', async (code) => {
                ...
                try {
                    ...
                    const status = code === 0 ? performance_1.TestStatus.COMPLETED : performance_1.TestStatus.FAILED;
...
exports.default = loadTestService;
