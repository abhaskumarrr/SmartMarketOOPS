...
exports.EnhancedMarketRegimeDetector = exports.EnhancedMarketRegime = void 0;
...
var EnhancedMarketRegime;
(function (EnhancedMarketRegime) {
    EnhancedMarketRegime["TRENDING_BULLISH"] = "trending_bullish";
    EnhancedMarketRegime["TRENDING_BEARISH"] = "trending_bearish";
    EnhancedMarketRegime["SIDEWAYS"] = "sideways";
    EnhancedMarketRegime["VOLATILE"] = "volatile";
    EnhancedMarketRegime["BREAKOUT_BULLISH"] = "breakout_bullish";
    EnhancedMarketRegime["BREAKOUT_BEARISH"] = "breakout_bearish";
    EnhancedMarketRegime["CONSOLIDATION"] = "consolidation";
    EnhancedMarketRegime["ACCUMULATION"] = "accumulation";
    EnhancedMarketRegime["DISTRIBUTION"] = "distribution";
})(EnhancedMarketRegime || (exports.EnhancedMarketRegime = EnhancedMarketRegime = {}));
class EnhancedMarketRegimeDetector {
    constructor(deltaService) {
        this.regimeHistory = new Map();
        this.changeDetectionWindow = 50; // Number of periods for change detection
        this.cusumThreshold = 5.0; // CUSUM threshold for regime change
    ...
    async detectRegime(symbol) {
        try {
            logger_1.logger.info(`🔍 Enhanced regime detection for ${symbol}`);
            ...
            const tradingRecommendations = this.generateTradingRecommendations(currentRegime, volatilityMetrics, trendStrength);
            ...
            logger_1.logger.info(`✅ Regime detected: ${currentRegime} (${(confidence * 100).toFixed(1)}% confidence)`);
            logger_1.logger.info(`📊 Strategy: ${tradingRecommendations.strategy_type}, Risk: ${tradingRecommendations.risk_multiplier}x`);
            return analysis;
        }
        catch (error) {
            logger_1.logger.error(`❌ Error detecting regime for ${symbol}:`, error);
            throw error;
        }
    }
    /**
    ...
    calculateVolatilityMetrics(data) {
        ...
        if (hourlyData?.candles && hourlyData.candles.length > 20) {
            ...
            const returns = candles.map((candle, i) => i > 0 ? (candle.close - candles[i - 1].close) / candles[i - 1].close : 0).slice(1);
            const meanReturn = returns.reduce((sum, r) => sum + r, 0) / returns.length;
            price_volatility = Math.sqrt(returns.reduce((sum, r) => sum + Math.pow(r - meanReturn, 2), 0) / returns.length);
            // Volume volatility
            const volumes = candles.map(c => c.volume);
            const meanVolume = volumes.reduce((sum, v) => sum + v, 0) / volumes.length;
            volume_volatility = Math.sqrt(volumes.reduce((sum, v) => sum + Math.pow(v - meanVolume, 2), 0) / volumes.length) / meanVolume;
        }
        return {
            atr_normalized,
            price_volatility,
     ...
     */
    calculateTrendStrengthMetrics(data, mtfAnalysis) {
        const hourlyData = data.timeframes['1h'];
        let adx = 0;
        let ma_slope = 0;
     ...
     */
    classifyRegime(mtfAnalysis, volatility, trendStrength) {
     ...
     */
    calculateRegimeConfidence(mtfAnalysis, volatility, trendStrength) {
     ...
     */
    detectRegimeChange(symbol, currentRegime) {
     ...
     */
    estimateRegimeDuration(regime, volatility) {
        const baseDurations = {
     ...
     */
    generateTradingRecommendations(regime, volatility, trendStrength) {
     ...
     */
    updateRegimeHistory(symbol, analysis) {
...
exports.EnhancedMarketRegimeDetector = EnhancedMarketRegimeDetector;
//# sourceMappingURL=EnhancedMarketRegimeDetector.js.map