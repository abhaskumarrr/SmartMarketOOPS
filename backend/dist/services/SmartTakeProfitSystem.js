...
exports.SmartTakeProfitSystem = void 0;
...
class SmartTakeProfitSystem {
     ...
     */
    async calculateTakeProfit(position, config = {}) {
        try {
            const finalConfig = { ...this.defaultConfig, ...config };
            logger_1.logger.info(`🎯 Calculating smart take profit for ${position.symbol}`);
            // Get market analysis
            ...
            const optimizedLevels = this.optimizeTakeProfitLevels(allLevels, finalConfig, position);
            ...
            const totalConfidence = this.calculateTotalConfidence(optimizedLevels, mtfAnalysis, regimeAnalysis);
            const smartTakeProfit = {
            ...
            logger_1.logger.info(`✅ Smart take profit calculated: ${optimizedLevels.length} levels`);
            logger_1.logger.info(`📊 Strategy: ${strategyType}, Confidence: ${(totalConfidence * 100).toFixed(1)}%`);
            return smartTakeProfit;
        }
        catch (error) {
            logger_1.logger.error(`❌ Error calculating smart take profit for ${position.symbol}:`, error);
            throw error;
        }
    }
    /**
     * Calculate support/resistance levels
     */
    calculateSupportResistanceLevels(marketData, position) {
        const levels = [];
        const dailyData = marketData.timeframes['1d'];
        const hourlyData = marketData.timeframes['1h'];
        if (!dailyData?.candles || !hourlyData?.candles)
            return levels;
     ...
     */
    calculateFibonacciLevels(marketData, position) {
        const levels = [];
        const hourlyData = marketData.timeframes['1h'];
        if (!hourlyData?.candles || hourlyData.candles.length < 50)
            return levels;
     ...
     * Calculate trend-based targets
     */
    calculateTrendTargets(position, mtfAnalysis) {
        const levels = [];
        ...
        if (!isAligned || trendStrength < 0.5)
            return levels;
        // Calculate trend-based targets
     ...
     * Calculate regime-adjusted targets
     */
    calculateRegimeTargets(position, regimeAnalysis, config) {
        const levels = [];
     ...
     */
    optimizeTakeProfitLevels(allLevels, config, position) {
        // Sort by distance
        ...
        for (const level of allLevels) {
            ...
            if (level.distance_percent - lastDistance >= config.min_target_distance) {
                ...
                if (optimizedLevels.length >= config.max_levels)
                    break;
            }
        }
     ...
     */
    determineStrategyType(regimeAnalysis, mtfAnalysis) {
     ...
     * Create execution plan
     */
    createExecutionPlan(levels, strategyType) {
     ...
     * Calculate total confidence
     */
    calculateTotalConfidence(levels, mtfAnalysis, regimeAnalysis) {
        if (levels.length === 0)
            return 0;
    ...
    // Helper methods
    findResistanceLevels(highs, currentPrice) {
        const levels = [];
    ...
    findSupportLevels(lows, currentPrice) {
        const levels = [];
    ...
    assessSupportResistanceQuality(marketData) {
        // Simplified quality assessment
        return 0.7; // Default moderate quality
    }
}
exports.SmartTakeProfitSystem = SmartTakeProfitSystem;
//# sourceMappingURL=SmartTakeProfitSystem.js.map