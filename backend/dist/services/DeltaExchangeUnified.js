...
class DeltaExchangeUnified extends events_1.EventEmitter {
        ...
        try {
            ...
            if (error.response) {
                ...
                if (error.response.data?.error?.context?.schema_errors) {
                    logger_1.logger.error('🔍 DETAILED SCHEMA ERRORS:');
                    logger_1.logger.error(JSON.stringify(error.response.data.error.context.schema_errors, null, 2));
                }
            }
            throw error;
        }
        ...
        try {
            ...
            for (const asset of assets) {
                try {
                    ...
                    logger_1.logger.warn(`Failed to get positions for ${asset}:`, error instanceof Error ? error.message : 'Unknown error');
            ...
            logger_1.logger.error('Error getting all positions:', error);
            throw error;
        }
    }
    /**
    ...
    async placeOrder(orderRequest) {
        ...
        try {
            ...
            if (response.success) {
                ...
                logger_1.logger.info(`✅ Order placed successfully: ${order.side} ${order.size} ${symbol} @ ${order.limit_price || 'market'}`);
     ...
     */
    convertToContractUnits(decimalSize, symbol) {
    ...
    async getMarketData(symbol) {
        ...
        try {
            ...
            if (response.success) {
                ...
                logger_1.logger.debug(`Raw ticker data for ${symbol}:`, ticker);
            ...
            logger_1.logger.error(`Error getting live market data for ${symbol}:`, error);
     ...
     */
    async getMultiTimeframeData(symbol, timeframes = ['1m', '5m', '15m', '1h', '4h', '1d']) {
        const result = {
            symbol,
            timeframes: {}
        };
        ...
        for (const timeframe of timeframes) {
            try {
                ...
                logger_1.logger.error(`Failed to get data for ${symbol} ${timeframe}:`, error);
     ...
     */
    getCachedCandleData(symbol, timeframe) {
        return this.candleCache.get(symbol)?.get(timeframe);
    }
    /**
     * Get cached indicators
     */
    getCachedIndicators(symbol, timeframe) {
...
//# sourceMappingURL=DeltaExchangeUnified.js.map