...
exports.DynamicTakeProfitManager = void 0;
...
class DynamicTakeProfitManager {
     ...
     */
    generateDynamicTakeProfitLevels(config) {
        const assetConfig = this.assetConfigs.get(config.asset);
        if (!assetConfig) {
            logger_1.logger.warn(`No config found for ${config.asset}, using default`);
        ...
        logger_1.logger.info(`🎯 Generating dynamic take profit for ${config.asset}:`);
        logger_1.logger.info(`   Market Regime: ${config.marketRegime.type} (${config.marketRegime.strength}%)`);
        logger_1.logger.info(`   Base Risk-Reward: ${baseRiskReward.toFixed(2)}:1`);
        const levels = [];
        ...
        levels.forEach((level, i) => {
            logger_1.logger.info(`     Level ${i + 1}: ${level.percentage}% at $${level.priceTarget.toFixed(2)} (${level.riskRewardRatio.toFixed(2)}:1)`);
        });
        return levels;
    }
    /**
     ...
     */
    calculateBaseRiskReward(config, assetConfig) {
     ...
     */
    calculateTargetPrice(config, stopLossDistance, riskReward) {
     ...
     */
    applyMomentumAdjustments(levels, config) {
     ...
     */
    applyVolumeAdjustments(levels, config) {
     ...
     */
    generateDefaultTakeProfitLevels(config) {
        const stopLossDistance = Math.abs(config.entryPrice - config.stopLoss);
        return [
            {
                percentage: 25,
     ...
     */
    updateTrailingTakeProfits(levels, currentPrice, config) {
     ...
     */
    checkTakeProfitExecution(levels, currentPrice, config) {
     ...
     */
    calculateBreakevenStop(config, firstProfitHit) {
        if (!firstProfitHit) {
...
exports.DynamicTakeProfitManager = DynamicTakeProfitManager;
//# sourceMappingURL=dynamicTakeProfitManager.js.map