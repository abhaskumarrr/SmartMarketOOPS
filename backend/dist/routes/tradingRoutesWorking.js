/**
 * Working Trading Routes (JavaScript)
 * Delta Exchange India trading integration endpoints
 */
const express = require('express');
const DeltaExchangeServiceWorking = require('../services/deltaExchangeServiceWorking');
...
const initializeDeltaService = () => {
    if (!deltaService) {
        ...
        if (!credentials.apiKey || !credentials.apiSecret) {
            console.error('❌ Delta Exchange credentials not configured');
            throw new Error('Delta Exchange API credentials are required');
        }
        deltaService = new DeltaExchangeServiceWorking(credentials);
        console.log('🔄 Delta Exchange service initialized with real credentials');
        console.log(`🌐 Environment: ${credentials.testnet ? 'TESTNET' : 'PRODUCTION'}`);
    }
    return deltaService;
};
/**
...
router.get('/products/:symbol', async (req, res) => {
    try {
        ...
        console.error(`Error getting product ${req.params.symbol}:`, error);
        res.status(500).json({
            success: false,
...
router.delete('/orders/:orderId', async (req, res) => {
    try {
        ...
        const success = await service.cancelOrder(parseInt(product_id), parseInt(orderId));
        if (success) {
            res.json({
                success: true,
...
router.get('/orders', async (req, res) => {
    try {
        ...
        const orders = await service.getOpenOrders(product_id ? parseInt(product_id) : undefined);
        res.json({
            success: true,
            data: orders,
...
//# sourceMappingURL=tradingRoutesWorking.js.map