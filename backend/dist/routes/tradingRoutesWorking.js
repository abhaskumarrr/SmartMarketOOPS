...
function createDeltaSignature(method, path, queryString, body, timestamp) {
    ...
    const signature = crypto_1.default.createHmac('sha256', DELTA_API_SECRET).update(message).digest('hex');
    console.log(`- Signature: ${signature}`);
    return signature;
}
// Helper function to make authenticated Delta Exchange API calls
async function makeDeltaApiCall(method, path, params = {}, data = null) {
...
router.post('/place-trade-with-tpsl', async (req, res) => {
    try {
        const { symbol = 'BTCUSD', side = 'buy', size = '0.01', order_type = 'market_order', take_profit_percentage = 2.0, stop_loss_percentage = 1.0 } = req.body;
...
router.post('/activate-bot', async (req, res) => {
    try {
        const { name = 'SmartMarketOOPS Bot', strategy = 'momentum_scalping', symbols = ['BTCUSD', 'ETHUSD'], risk_per_trade = 2.0, take_profit = 2.0, stop_loss = 1.0, max_positions = 3, enabled = true } = req.body;
