#!/usr/bin/env python3
"""
Modern Real-time Market Data WebSocket Server
Compatible with websockets 15.0.1+ API
"""
...
async def initialize():
    ...
    logger.info("✅ Modern Market WebSocket Server initialized")
...
async def register_client(websocket):
    ...
    try:
        await websocket.send(json.dumps({
            ...
            'data': {'status': 'connected', 'server': 'modern-market-data'}
        }))
...
async def handle_client(websocket):
    """Handle individual client connections - MODERN API SIGNATURE"""
        ...
        try:
            if clients:
                ...
                if market_data_list:
                    ...
                    logger.info(f"📊 Broadcasted market data for {len(market_data_list)} symbols to {len(clients)} clients")
...
async def start_server():
    ...
    logger.info(f"🚀 Starting Modern Market WebSocket server on localhost:3001")
    ...
    try:
        # Start WebSocket server with MODERN API (no path parameter)
        async with websockets.serve(
            handle_client,  # Function with single websocket parameter
            "localhost",
            3001,
            ping_interval=30,
            ping_timeout=10
        ) as server:
            logger.info("✅ Modern Market WebSocket server started")
        ...
        await cleanup()
        logger.info("🏁 Modern Market WebSocket server stopped")
