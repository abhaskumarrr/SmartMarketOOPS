...
class MinimalBacktester {
  ...
  private getETHTrendFactor(date: Date): number {
    ...
    if (month >= 0 && month <= 2) return 0.002;  // Q1 bull run
    if (month >= 3 && month <= 5) return -0.001; // Q2 correction
    if (month >= 6 && month <= 8) return 0;      // Q3 consolidation
    return 0.0015; // Q4 rally
  }

  private generateAdvancedFilteredSignal(data: any, dayIndex: number): any {
  ...
  private executeFilteredTrade(data: any, signal: any): Trade {
  ...
  private exitTrade(trade: Trade, exitData: any): void {
